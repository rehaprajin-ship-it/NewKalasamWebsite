...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  const { user, loading, isAdmin, login, logout, loginError, inAppBrowser, clearLoginError } = useAuth();
    ...
    return (
      <div className="min-h-screen bg-[#F7F8FA] flex items-center justify-center px-4">
        <div className="bg-white rounded-[18px] border border-gray-200/80 p-8 md:p-10 text-center max-w-md w-full shadow-lg relative">
          ...
          <p className="text-sm text-gray-500 mb-6">{COMPANY.name} — Control Center</p>

          {/* In-app browser warning */}
          {inAppBrowser && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-[12px] text-left">
              <p className="text-sm font-700 text-amber-800 mb-1">⚠️ In-App Browser Detected</p>
              <p className="text-xs text-amber-700 leading-relaxed">
                You&apos;re viewing this inside <strong>{inAppBrowser}</strong>&apos;s built-in browser. Google sign-in is blocked here by Google&apos;s security policy.
              </p>
              <p className="text-xs text-amber-700 mt-2 font-600">
                Tap the <strong>⋯</strong> menu (top-right) → &quot;Open in Browser&quot; to sign in.
              </p>
            </div>
          )}
          ...
          {/* Login error message */}
          {loginError && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-[12px] text-left">
          ...
          <button
            ...
            className={`w-full py-3 font-700 rounded-[12px] flex items-center justify-center gap-3 transition-colors shadow-md text-sm ${
              inAppBrowser
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] flex text-gray-800 font-sans admin-touch">
      ...
      {/* Collapsible Sidebar */}
      <aside className={`bg-[#128C7E] text-white flex flex-col transition-all duration-300 z-40 fixed lg:static inset-y-0 left-0 admin-safe-area ${isSidebarOpen ? 'w-[280px] lg:w-64 translate-x-0' : 'w-0 lg:w-20 -translate-x-full lg:translate-x-0 overflow-hidden'
        }`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3.5 px-4 py-3 min-h-[48px] rounded-[12px] text-sm font-600 transition-all duration-200 ${isActive
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Headerbar */}
        <header className="bg-white/70 backdrop-blur-md border-b border-gray-200/80 sticky top-0 z-30 px-4 lg:px-6 py-3 lg:py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
              className="p-2.5 lg:p-1.5 rounded-[8px] bg-gray-100 hover:bg-gray-200 text-gray-500 hover:text-gray-700 transition-colors cursor-pointer min-w-[44px] min-h-[44px] flex items-center justify-center"
            >
        ...
        <div className="flex-1 overflow-y-auto admin-safe-area">
          {children}
        </div>
      </div>
    </div>
